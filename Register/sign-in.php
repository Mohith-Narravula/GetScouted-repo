...
<html lang="en">
  <head>
    ...
    <title>Get Scouted Sign-in</title>
...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
    ...
    {
        $sql = "SELECT * from account where `User Name` = '$username'";
        $result = $pdo->query($sql);

        if ($result->rowCount() == 1)
        {
            $user = getSingleValue($pdo, "SELECT `User Name` FROM account WHERE `User Name`=?", [$username]);
            $user = $username;
            $pass = getSingleValue($pdo, "SELECT `Password` FROM account WHERE `User Name`=?", [$username]);
...
<main class="form-signin w-100 m-auto">
  <form  action='' method='post'>
    ...
    <h2 class="h3 mb-3 fw-normal text-light">Enter Credentials</h2>
